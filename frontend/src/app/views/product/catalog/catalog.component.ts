...
import {DIAMETER_FROM, DIAMETER_TO, HEIGHT_FROM, HEIGHT_TO} from "../../../shared/constants/constants";
...
export class CatalogComponent implements OnInit {
          ...
          .subscribe(params => {
            ...
            this.activeParams.types.forEach(url => {
              for (let i = 0; i < this.categoriesWithTypes.length; i++) {
                const foundType = this.categoriesWithTypes[i].types
                  .find(type => type.url === url);
                if (foundType) {
            ...
            if (this.activeParams.diameterFrom) {
              this.appliedFilters.push({
                ...
                urlParam: DIAMETER_FROM
              });
            }
            if (this.activeParams.diameterTo) {
              this.appliedFilters.push({
                name: 'Диаметр: до ' + this.activeParams.diameterTo + ' см',
                urlParam: DIAMETER_TO
              });
            }
  ...
  removeAppliedFilter(appliedFilter: AppliedFilterType) {
    if (appliedFilter.urlParam === HEIGHT_FROM || appliedFilter.urlParam === HEIGHT_TO
      || appliedFilter.urlParam === DIAMETER_FROM || appliedFilter.urlParam === DIAMETER_TO) {
